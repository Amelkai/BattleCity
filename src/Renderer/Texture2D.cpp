...
namespace Renderer {
	...
	{
		m_subTextures.emplace(std::move(name), SubTexeture2D(leftBottomUV, rightTopUV));
	}

	const Texture2D::SubTexeture2D& Texture2D::getSubTexture(const std::string& name) const
	{
		...
		const static SubTexeture2D defaultSubtexture;
